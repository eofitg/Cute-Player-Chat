...
public class ChatSuffix extends ChatAffix {
    protected static String textGet = "tips.success.get.affix.suffix.text";
    protected static String successGet = "tips.success.get.affix.suffix.partial";
    protected static String failGet = "tips.fail.get.affix.suffix.partial";
    protected static String successGetAll = "tips.success.get.affix.suffix.all";
    protected static String failGetAll = "tips.fail.get.affix.suffix.all";
    protected static String successSet = "tips.success.set.affix.suffix";
    protected static String successDelete = "tips.success.delete.affix.suffix.partial";
    protected static String failDelete = "tips.fail.delete.affix.suffix.partial";
    protected static String successDeleteAll = "tips.success.delete.affix.suffix.all";
    protected static String failDeleteAll = "tips.fail.delete.affix.suffix.all";
    ...
    public static void getSuffix (CommandSender sender, String args[]) {
        ...
        if (args.length == 0) { // 等于0，代表没有额外指定玩家名字，那么查看自己的(/getsuf)
            if (!SuffixReader.getSuffix(playerName).equals("")&&!SuffixReader.getSuffix(playerName).equals("EMPTY")) {
                sender.sendMessage(format(textGet, playerName, SuffixReader.getSuffix(playerName)));
            ...
            for (int i=0; i<args.length; i++) {
                ...
                if (!SuffixReader.getSuffix(_playerName).equals("")&&!SuffixReader.getSuffix(_playerName).equals("EMPTY")) {
                    sender.sendMessage(format(textGet, _playerName, SuffixReader.getSuffix(_playerName)));
            ...
            if (cantGetPlayerNames.equals("")) {
                ...
                // sendMessage2 = "§4" + cantGetPlayerNames + "还没有设置聊天后缀！";
            }
    ...
    public static void setSuffix (CommandSender sender, String args[], String suffix) {
        ...
        String playerNames;
        if (args.length == 1) { // 等于1，代表没有额外指定玩家名字，那么修改自己的(/setsuf <suffix>)
            SuffixReader.setSuffix(playerName, suffix);
            sender.sendMessage(format(successSet, playerName, suffix));
            // sender.sendMessage("§a" + playerName + "的聊天后缀已修改为：" + suffix);
        } else {
            // 大于1，代表额外指定了玩家名字
            // /setsuf [playerName_1] ... [playerName_n] <suffix>
            playerNames = args[0];
            ...
            sender.sendMessage(format(successSet, playerNames, suffix));
    ...
    public static void deleteSuffix (CommandSender sender, String args[]) {
        ...
        String sendMessage1, sendMessage2;
        if (args.length == 0) { // 等于0，代表没有额外指定玩家名字，那么删除自己的(/delsuf)
            ...
            // 大于0，代表额外指定了玩家名字
            // /delsuf [playerName_1] ... [playerName_n]
            for (int i=0; i<args.length; i++) {
                String _playerName = args[i];
            ...
            if (canDelPlayerNames.equals("")) {
                ...
                // sendMessage1 = "§7" + canDelPlayerNames + "的聊天后缀已删除！";
            }
            if (cantDelPlayerNames.equals("")) {
                ...
                // sendMessage2 = "§4" + cantDelPlayerNames + "还没有设置聊天后缀！";
            }
            sender.sendMessage(sendMessage1);
            sender.sendMessage(sendMessage2);
        }
